defmodule SovereignSoulEngineWeb.AcpCharacterLive do
  ...
  defp emotional_draft_from(emotional) do
    Map.new(emotion_fields(), fn {_label, key, _color} ->
      {Atom.to_string(key), Map.get(emotional, key) || 0}
    end)
  end
  ...
  defp load_tab_data(socket, "arcs", _id) do
    ...
    |> assign(
      :forgiveness_arcs,
      Souls.list_active_forgiveness_arcs_for_character(socket.assigns.id)
    )
  end
  ...
  defp load_tab_data(socket, "social", id) do
    ...
    existing_target_ids =
      Enum.map(relationships, fn {rel, _target} -> rel.target_character_id end)
  ...
  def handle_event("update_tom_draft", params, socket) do
    socket =
      ...
      |> assign(
        :tom_draft_certainty,
        parse_int(params["certainty"], socket.assigns.tom_draft_certainty)
      )
      |> assign(:tom_draft_assumption, Map.get(params, "is_assumption", "true") == "true")
      |> assign(
        :tom_draft_subject_id,
        Map.get(params, "subject_id", socket.assigns.tom_draft_subject_id)
      )
  ...
  def handle_event("save_emotional_state", _params, socket) do
    case Souls.update_emotional_state(socket.assigns.emotional, socket.assigns.emotional_draft) do
      {:ok, _} ->
        {:noreply,
         socket
         |> put_flash(:info, "Emotional state saved.")
  ...
  def handle_event("save_soul_profile", _params, socket) do
    case Souls.update_soul_profile(socket.assigns.soul, socket.assigns.soul_draft) do
      {:ok, _} ->
        {:noreply,
         socket
         |> put_flash(:info, "Soul profile saved.")
         |> load_character_data(socket.assigns.id)}
  ...
  def handle_event("save_goal", %{"goal_id" => goal_id} = params, socket) do
    ...
    case goal && Souls.update_goal(goal, attrs) do
      {:ok, _} ->
        {:noreply,
         socket |> put_flash(:info, "Goal updated.") |> load_character_data(socket.assigns.id)}
  ...
  def handle_event("add_goal", %{"goal" => goal_text} = params, socket) do
    if String.trim(goal_text) != "" do
      ...
      case Souls.create_goal(attrs) do
        {:ok, _} ->
          {:noreply,
           socket |> put_flash(:info, "Goal added.") |> load_character_data(socket.assigns.id)}
  ...
  def handle_event("save_grief_arc", %{"arc_id" => arc_id} = params, socket) do
    ...
    case arc && Souls.update_grief_arc(arc, attrs) do
      {:ok, _} ->
        {:noreply,
         socket
         |> put_flash(:info, "Grief arc updated.")
         |> load_character_data(socket.assigns.id)}
    ...
    if String.trim(subject) != "" do
      ...
      case Souls.create_grief_arc(attrs) do
        {:ok, _} ->
          {:noreply,
           socket
           |> put_flash(:info, "Grief arc added.")
           |> load_character_data(socket.assigns.id)}
  ...
  def handle_event("save_relationship", %{"relationship_id" => rel_id} = params, socket) do
    rel =
      Enum.find_value(socket.assigns.relationships, fn {rel, _target} ->
        rel.id == rel_id && rel
      end)
    ...
    try do
      ...
      Ecto.StaleEntryError ->
        {:noreply,
         ...
         |> put_flash(
           :error,
           "Someone else changed this relationship first — reloaded with the latest values."
         )
         |> load_character_data(socket.assigns.id)}
    end
  end
  ...
  defp cog_badge(score) when score >= 40,
    do: {"bg-yellow-500/20 text-yellow-400 border-yellow-500/30", "Medium"}
    ...
    <div class="min-h-screen bg-gray-950 text-gray-100">
      ...
      <div class="px-6 py-6">
        ...
        <div :if={@tab == "vitals"}>
          <div class="grid grid-cols-3 gap-4 mb-6">
            ...
            <div class="p-4 rounded-xl border border-gray-800 bg-gray-900">
              ...
              <%= if @emotional do %>
                <form
                  phx-change="update_emotional_draft"
                  phx-submit="save_emotional_state"
                  class="space-y-2.5"
                >
                    ...
                    <div>
                      <div class="flex justify-between text-xs mb-0.5">
                        ...
                        <span class="text-gray-300 font-mono">
                          {Map.get(@emotional_draft, key_str, 0)}
                        </span>
                      </div>
                  ...
                  <button
                    type="submit"
                    class="mt-1 w-full text-xs px-3 py-1.5 rounded bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 transition-colors"
                  >
                    ...
                    <div>
                      ...
                      <div class="h-1.5 w-full rounded-full bg-gray-800 overflow-hidden">
                        <div
                          ...
                          style={"width: #{Map.get(@somatic, key) || 0}%; background-color: #{color}"}
                        >
                        </div>
                      </div>
                    </div>
            ...
            <div class="p-4 rounded-xl border border-gray-800 bg-gray-900">
              ...
              <div class="flex items-center gap-3 mb-3">
                ...
                <span class={"text-xs px-2 py-0.5 rounded-full border font-bold #{cog_class}"}>
                  {cog_label}
                </span>
              </div>
              ...
              <div class="h-2 w-full rounded-full bg-gray-800 overflow-hidden mb-3">
                <div
                  class={[
                    ...
                    @cog_score >= 70 && "bg-red-500",
                    @cog_score >= 40 && @cog_score < 70 && "bg-yellow-500",
                    @cog_score < 40 && "bg-green-500"
                  ]}
                ...
                <ul class="space-y-1">
                  <%= for stressor <- @cog_stressors do %>
                    <li class="text-xs text-gray-500 flex items-center gap-1.5">
                      <span class="w-1 h-1 rounded-full bg-yellow-500 shrink-0"></span> {stressor}
                    </li>
                  <% end %>
                </ul>
              <% else %>
            ...
            <div class="p-4 rounded-xl border border-gray-800 bg-gray-900 mb-4">
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs text-gray-400 font-mono">
                  {@soul.social_stamina} / {@soul.stamina_max}
                </span>
              </div>
            ...
            <div class="p-4 rounded-xl border border-gray-800 bg-gray-900">
              ...
              <form phx-change="update_soul_draft" phx-submit="save_soul_profile" class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label class="block text-[10px] text-gray-500 mb-1 uppercase tracking-wide">
                      Attachment Style
                    </label>
                    <select
                      name="attachment_style"
                      class="w-full bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-blue-500"
                    >
                      <%= for style <- ~w(secure anxious avoidant disorganized) do %>
                        <option value={style} selected={@soul_draft["attachment_style"] == style}>
                  ...
                  <div>
                    ...
                    >
                      <%= for style <- ~w(none dry sarcastic warm dark absurdist) do %>
                        <option value={style} selected={@soul_draft["humor_style"] == style}>
                          {String.capitalize(style)}
                        </option>
                      <% end %>
                    </select>
                ...
                <div>
                  <div class="flex justify-between text-[10px] text-gray-500 mb-1 uppercase tracking-wide">
                    ...
                    <span class="font-mono normal-case">
                      {@soul_draft["emotional_susceptibility"]}%
                    </span>
                  </div>
                  ...
                  <input
                    ...
                    value={@soul_draft["emotional_susceptibility"]}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="2"
                    class="w-full bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-blue-500"
                  >{@soul_draft["speech_style"]}</textarea>
                </div>

                <div>
                  <label class="block text-[10px] text-gray-500 mb-1.5 uppercase tracking-wide">
                  ...
                  <div class="grid grid-cols-3 gap-1.5">
                    <%= for %{key: trait, label: label, blurb: blurb} <- TraitCatalog.all() do %>
                      ...
                      >
                        <div class={"w-1.5 h-1.5 rounded-full shrink-0 #{if active, do: "bg-purple-400", else: "bg-gray-600"}"}>
                        </div>
                        {label}
                      </button>
                    <% end %>
                ...
                <button
                  type="submit"
                  class="w-full text-xs px-3 py-1.5 rounded bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 transition-colors"
                >
                  Save Soul Profile
                </button>
              </form>
        ...
        <div :if={@tab == "timeline"}>
          ...
          <div
            :if={!is_map_key(assigns, :timeline_events) or @timeline_events == []}
        ...
        <div :if={@tab == "memory"}>
          ...
          <div
            :if={!is_map_key(assigns, :memories) or @memories == []}
            class="text-center py-12 text-gray-600"
          >
                ...
                <div class="flex items-start gap-3">
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <%= if Map.get(mem, :is_consolidated, false) do %>
                        <span class="text-[10px] px-2 py-0.5 rounded bg-purple-500/15 text-purple-400 border border-purple-500/20">
                          merged
                        </span>
                      <% end %>

                      <span class={"text-[10px] font-semibold #{valence_class(Map.get(mem, :valence, 0))}"}>
                        {if (Map.get(mem, :valence, 0) || 0) > 0,
                          do: "positive",
                          else:
                            if((Map.get(mem, :valence, 0) || 0) < 0, do: "negative", else: "neutral")}
                      </span>
                    </div>
                  ...
                  <div class="shrink-0 text-right">
                    ...
                    <div class="text-[10px] text-gray-600">
                      recalled: {Map.get(mem, :recall_count, 0)}x
                    </div>

                    <div class="text-[10px] text-gray-600">
                      {format_dt(Map.get(mem, :last_recalled_at))}
                    </div>
                  </div>
                </div>
        ...
        <div :if={@tab == "beliefs"}>
          ...
          <div class="mb-6">
            ...
            <div
              :if={!is_map_key(assigns, :beliefs) or @beliefs == []}
            ...
            <div class="grid grid-cols-2 gap-3">
              <%= for belief <- Map.get(assigns, :beliefs, []) do %>
                <div class={[
                  "p-3 rounded-lg border bg-gray-900",
                  if(belief.is_challenged, do: "border-orange-500/40", else: "border-gray-800")
                ]}>
                  <div class="flex items-center justify-between gap-2 mb-2">
                    <span class="text-[10px] px-2 py-0.5 rounded bg-purple-500/15 text-purple-400 border border-purple-500/20 font-semibold">
                      {belief.domain}
                    </span>
                  ...
                  <div class="text-[10px] text-gray-600 mt-1">
                    conviction: {belief.conviction}/100
                  </div>
                </div>
              <% end %>
            </div>
          ...
          <div class="mb-6">
            ...
            <div
              :if={!is_map_key(assigns, :triggers) or @triggers == []}
              class="text-xs text-gray-600 italic"
            >
            ...
            <div class="space-y-2">
              <%= for t <- Map.get(assigns, :triggers, []) do %>
                <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-800 bg-gray-900">
                  <span class="text-xs font-semibold text-gray-300 px-2 py-0.5 rounded bg-gray-800">
                    {t.topic}
                  </span>
                  <span class={"text-[10px] px-2 py-0.5 rounded font-semibold #{reaction_badge_class(t.reaction_type)}"}>
                    {String.replace(t.reaction_type, "_", " ")}
                  </span>
                  <span class="text-[10px] text-gray-500">intensity: {t.intensity_modifier}</span>
                  <span :if={t.flavor_text} class="text-[10px] text-gray-500 italic truncate flex-1">
                    "{t.flavor_text}"
                  </span>
                </div>
              <% end %>
          ...
          <div class="mb-6">
            ...
            <div
              :if={!is_map_key(assigns, :moral_lines) or @moral_lines == []}
              class="text-xs text-gray-600 italic"
            >
              No moral lines recorded
            </div>

            <div class="space-y-2">
              <%= for ml <- Map.get(assigns, :moral_lines, []) do %>
                <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-800 bg-gray-900">
                  <div class="flex-1"><span class="text-sm text-gray-200">{ml.principle}</span></div>

                  <%= if ml.will_refuse_when_violated do %>
                    <span class="text-[10px] px-2 py-0.5 rounded bg-red-500/20 text-red-400 border border-red-500/30 font-bold">
                      WILL REFUSE
                    </span>
                  <% end %>
          ...
          <%!-- Secrets --%>
          <div>
            <h3 class="text-xs font-bold text-rose-400 uppercase tracking-wide mb-3">
              Secrets (Inspector Only)
            </h3>

            <div
              :if={!is_map_key(assigns, :secrets) or @secrets == []}
              class="text-xs text-gray-600 italic"
            >
              No secrets recorded
            </div>
            ...
            <div class="space-y-2">
              <%= for s <- Map.get(assigns, :secrets, []) do %>
                <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-800 bg-gray-900">
                  <span class={"text-[10px] px-2 py-0.5 rounded border font-bold shrink-0 #{risk_badge_class(s.risk_level)}"}>
                    {s.risk_level}
                  </span>
        ...
        <div :if={@tab == "arcs"}>
          <div class="grid grid-cols-2 gap-6">
            <%!-- Grief Arcs --%>
            <div>
              <h3 class="text-xs font-bold text-indigo-400 uppercase tracking-wide mb-4">
                Grief Arcs
              </h3>

              <div :if={@grief_arcs == []} class="text-xs text-gray-600 italic">
              ...
              <div class="space-y-4">
                <%= for arc <- @grief_arcs do %>
                  <div class="p-4 rounded-xl border border-indigo-500/20 bg-gray-900">
                    ...
                    <div class="flex items-center gap-1 mb-3">
                      ...
                      <%= for {stage, idx} <- Enum.with_index(grief_stages()) do %>
                        <div class="flex items-center">
                          ...
                          <div
                            :if={idx < length(grief_stages()) - 1}
                            class={"h-0.5 w-4 #{if idx < active_idx, do: "bg-indigo-700", else: "bg-gray-800"}"}
                          >
                          </div>
                        </div>
                      <% end %>
                    ...
                    <div class="text-[10px] text-gray-600 mt-1">
                      intensity: {arc.intensity} | triggered: {format_dt(arc.triggered_at)}
                    </div>

                    <details class="mt-2">
                      <summary class="text-[10px] text-gray-600 uppercase tracking-wide cursor-pointer hover:text-gray-400 transition-colors">
                        Edit
                      </summary>
                      ...
                      <form phx-submit="save_grief_arc" class="mt-2 grid grid-cols-2 gap-2">
                        ...
                        >
                          <%= for s <- grief_stages() do %>
                            <option value={s} selected={arc.stage == s}>
                              {String.capitalize(s)}
                            </option>
                          <% end %>
                        </select>
                        ...
                        <button
                          type="submit"
                          class="col-span-2 text-xs px-3 py-1.5 rounded bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/30 transition-colors"
                        >
                          Save
                        </button>
                      </form>
              ...
              <form
                phx-submit="add_grief_arc"
                class="mt-4 p-3 rounded-lg border border-gray-800 bg-gray-900/50 grid grid-cols-2 gap-2"
              >
                <input
                  type="text"
                  name="subject"
                  placeholder="Subject (who/what was lost)"
                  required
                  class="col-span-2 bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-indigo-500"
                />
                <select
            ...
            <%!-- Forgiveness Arcs --%>
            <div>
              <h3 class="text-xs font-bold text-rose-400 uppercase tracking-wide mb-4">
                Forgiveness Arcs
              </h3>

              <div
                :if={!is_map_key(assigns, :forgiveness_arcs) or @forgiveness_arcs == []}
                class="text-xs text-gray-600 italic"
              >
                  ...
                  <div class="p-4 rounded-xl border border-rose-500/20 bg-gray-900">
                    ...
                    <div class="flex items-center gap-1 mb-3">
                      ...
                      <%= for {stage, idx} <- Enum.with_index(forgiveness_stages()) do %>
                        <div class="flex items-center">
                          ...
                          <div
                            :if={idx < length(forgiveness_stages()) - 1}
                            class={"h-0.5 w-4 #{if idx < active_idx, do: "bg-rose-700", else: "bg-gray-800"}"}
                          >
                          </div>
                        </div>
                      <% end %>
                    ...
                    <div class="flex items-center gap-3 mb-2">
                      ...
                      <span class={"flex items-center gap-1 text-[10px] font-semibold #{direction_color(arc.direction)}"}>
                        <.icon name={direction_icon(arc.direction)} class="size-3" /> {arc.direction}
                      </span>
                    </div>

                    <div class="h-1.5 w-full rounded-full bg-gray-800 overflow-hidden">
                      <div class="h-full rounded-full bg-rose-500" style={"width: #{arc.intensity}%"}>
                      </div>
                    </div>
                  </div>
                <% end %>
        ...
        <div :if={@tab == "goals"}>
          ...
          <div class="mb-6">
                ...
                <div class="p-4 rounded-xl border border-gray-800 bg-gray-900">
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex-1">
                      ...
                      <p :if={goal.current_step} class="text-xs text-gray-400 mt-1">
                        Next: {goal.current_step}
                      </p>
                    </div>
                  ...
                  <details class="mt-3">
                    ...
                    <form phx-submit="save_goal" class="mt-2 grid grid-cols-2 gap-2">
                      ...
                      <input
                        ...
                        name="blocker"
                        value={goal.blocker}
                        placeholder="Blocker (blank = none)"
                        class="col-span-2 bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-blue-500"
                      />
                      <input
                        type="number"
                      ...
                      <button
                        type="submit"
                        class="col-span-2 text-xs px-3 py-1.5 rounded bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 transition-colors"
                      >
                        Save
                      </button>
                    </form>
            ...
            <form
              phx-submit="add_goal"
              class="mt-3 p-3 rounded-lg border border-gray-800 bg-gray-900/50 grid grid-cols-2 gap-2"
            >
              <input
                type="text"
                name="goal"
              ...
              <input
                type="text"
                name="current_step"
                placeholder="Current step (optional)"
                class="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-blue-500"
              />
              <input
                type="number"
          ...
          <div class="mb-6">
            ...
            <div
              :if={!is_map_key(assigns, :desires) or @desires == []}
              class="text-xs text-gray-600 italic"
            >
              No desires recorded
            </div>
            ...
            <div class="space-y-2">
              <%= for d <- Map.get(assigns, :desires, []) do %>
                <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-800 bg-gray-900">
                  ...
                  <span class="text-[10px] px-2 py-0.5 rounded bg-green-500/15 text-green-400 border border-green-500/20 font-semibold">
                    {d.domain}
                  </span>
                  <div class="w-20">
                    <div class="h-1 rounded-full bg-gray-800 overflow-hidden">
                      <div class="h-full rounded-full bg-green-500" style={"width: #{d.urgency}%"}>
                      </div>
                    </div>
              ...
              <div class="mt-3 space-y-2">
                <%= for goal <- Map.get(assigns, :completed_goals, []) do %>
                  <div class="flex items-center gap-3 p-3 rounded-lg border border-gray-800 bg-gray-900/50">
                    <span class={"text-[10px] px-2 py-0.5 rounded border #{if goal.status == "achieved", do: "bg-green-500/20 text-green-400 border-green-500/30", else: "bg-gray-700 text-gray-500 border-gray-600"}"}>
                      {goal.status}
                    </span>
        ...
        <div :if={@tab == "social"}>
          ...
          <div
            :if={Map.get(assigns, :relationships, []) == []}
            class="text-xs text-gray-600 italic mb-4"
          >
            No outbound relationships recorded
          </div>
            ...
            <%= for {rel, target} <- Map.get(assigns, :relationships, []) do %>
              <div class="p-4 rounded-xl border border-gray-800 bg-gray-900">
                ...
                <div class="flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-gray-500 mb-1">
                  <span>Affinity {rel.affinity}</span> <span>Trust {rel.trust}</span>
                  <span>Respect {rel.respect}</span> <span>Fear {rel.fear}</span>
                ...
                <details class="mt-2">
                  ...
                  <form phx-submit="save_relationship" class="mt-2 space-y-2">
                    ...
                    <select
                      ...
                      class="w-full bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-cyan-500"
                    >
                      <%= for t <- ~w(acquaintance friend rival ally enemy family romantic mentor) do %>
                        <option value={t} selected={rel.relationship_type == t}>
                          {String.capitalize(t)}
                        </option>
                      <% end %>
                    </select>
                    ...
                    <button
                      type="submit"
                      class="w-full text-xs px-3 py-1.5 rounded bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/30 transition-colors"
                    >
                      Save
                    </button>
                  </form>
          ...
          <%= if Map.get(assigns, :relationship_candidates, []) != [] do %>
            <form
              ...
              class="mb-6 p-3 rounded-lg border border-gray-800 bg-gray-900/50 space-y-2"
            >
              <select
                name="target_id"
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-xs text-gray-100 focus:outline-none focus:border-cyan-500"
          ...
          <div
            :if={!is_map_key(assigns, :char_scenes) or @char_scenes == []}
            class="text-center py-12 text-gray-600"
          >
          ...
          <div class="space-y-3">
            <%= for scene <- Map.get(assigns, :char_scenes, []) do %>
              <div class="rounded-xl border border-gray-800 bg-gray-900 overflow-hidden">
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div class="text-[10px] text-gray-500">
                        {Enum.map(scene.participants, & &1.character.name) |> Enum.join(", ")} · {format_dt(
                          scene.inserted_at
                        )}
                      </div>
                    </div>
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <.icon
                      name={
                        if MapSet.member?(
                             Map.get(assigns, :expanded_scene_ids, MapSet.new()),
                             scene.id
                           ), do: "hero-chevron-up", else: "hero-chevron-down"
                      }
                ...
                <div
                  :if={MapSet.member?(Map.get(assigns, :expanded_scene_ids, MapSet.new()), scene.id)}
                  class="border-t border-gray-800 px-4 py-3 space-y-2 max-h-80 overflow-y-auto"
                >
                  <%= for msg <- scene.messages do %>
                    <div class="text-xs">
                      <span class="font-semibold text-amber-400">
                        {Enum.find_value(scene.participants, "?", fn p ->
                          if p.character_id == msg.character_id, do: p.character.name
                        end)}:
                      </span>
        ...
        <div :if={@tab == "theory_of_mind"}>
          <h2 class="text-sm font-bold text-gray-300 mb-4">
            Theory of Mind — What {@character.name} Believes About Others
          </h2>
          <%!-- Grouped knowledge --%>
          <div class="space-y-5 mb-8">
            <div
              :if={!is_map_key(assigns, :tom_grouped) or @tom_grouped == []}
              class="text-xs text-gray-600 italic"
            >
                  ...
                  <%= for entry <- entries do %>
                    <div class="flex items-center gap-3 p-2 rounded-lg bg-gray-800/60">
                      ...
                      <%= if entry.is_assumption do %>
                        <span class="text-[10px] px-1.5 py-0.5 rounded bg-yellow-500/15 text-yellow-500 border border-yellow-500/20">
                          assumption
                        </span>
                      <% end %>
                    </div>
          ...
          <div class="p-4 rounded-xl border border-cyan-500/20 bg-gray-900">
            <h3 class="text-xs font-bold text-cyan-400 uppercase tracking-wide mb-3">
              Add Knowledge Entry
            </h3>
            ...
            <form phx-change="update_tom_draft" phx-submit="save_tom_entry" class="space-y-3">
              ...
              >
                ...
                <%= for char <- Map.get(assigns, :tom_all_chars, []) do %>
                  <option value={char.id} selected={@tom_draft_subject_id == char.id}>
                    {char.name}
                  </option>
                <% end %>
              </select>
              <input
                type="text"
                name="known_fact"
                value={@tom_draft_fact}
                placeholder="Known fact or belief about this character"
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-cyan-500"
              />
              <div class="flex items-center gap-3">
                <input
              ...
              <button
                type="submit"
                class="px-4 py-2 rounded-lg bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 text-sm font-semibold hover:bg-cyan-500/30 transition-colors"
              >
                Save Entry
              </button>
              ...
              <%= if @tom_save_result == :error do %>
                <span class="text-xs text-red-400 ml-2">
                  Please select a subject and enter a fact.
                </span>
              <% end %>
            </form>
          </div>
