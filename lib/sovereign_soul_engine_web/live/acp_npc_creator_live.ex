defmodule SovereignSoulEngineWeb.AcpNpcCreatorLive do
  ...
  def handle_event("update_personality", params, socket) do
    socket =
      ...
      |> assign(
        :attachment_style,
        Map.get(params, "attachment_style", socket.assigns.attachment_style)
      )
      |> assign(:humor_style, Map.get(params, "humor_style", socket.assigns.humor_style))
      |> assign(
        :emotional_susceptibility,
        parse_int(params["emotional_susceptibility"], socket.assigns.emotional_susceptibility)
      )
  ...
  def handle_event("update_belief_draft", params, socket) do
    draft =
      ...
      |> Map.put(
        "conviction",
        parse_int(params["conviction"], socket.assigns.belief_draft["conviction"])
      )
    ...
    draft =
      ...
      |> Map.put(
        ...
        parse_int(
          ...
          socket.assigns.trigger_draft["intensity_modifier"]
        )
      )
  ...
  def handle_event("add_moral_line", _params, socket) do
    if String.trim(socket.assigns.moral_line_draft["principle"] || "") != "" do
      ...
      socket =
        socket
        |> assign(:moral_lines, moral_lines)
        |> assign(:moral_line_draft, %{
          "principle" => "",
          "will_refuse" => true,
          "action_types_blocked" => []
  ...
  def handle_event("update_secret_draft", params, socket) do
    draft =
      socket.assigns.secret_draft
      |> Map.merge(Map.take(params, ["secret_text", "risk_level", "domain"]))
  ...
  def handle_event("update_grief_draft", params, socket) do
    draft =
      ...
      |> Map.put(
        "intensity",
        parse_int(params["intensity"], socket.assigns.grief_draft["intensity"])
      )
  ...
  def handle_event("update_forgiveness_draft", params, socket) do
    draft =
      ...
      |> Map.put(
        "intensity",
        parse_int(params["intensity"], socket.assigns.forgiveness_draft["intensity"])
      )
  ...
  def handle_event("remove_forgiveness_arc", %{"index" => idx_str}, socket) do
    ...
    {:noreply,
     assign(socket, :forgiveness_arcs, List.delete_at(socket.assigns.forgiveness_arcs, idx))}
  end
  ...
  def handle_event("update_tom_draft", params, socket) do
    draft =
      ...
      |> Map.put(
        "certainty",
        parse_int(params["certainty"], socket.assigns.tom_draft["certainty"])
      )
  ...
  def handle_event("update_stamina", params, socket) do
    socket =
      ...
      |> assign(
        :social_stamina,
        parse_int(params["social_stamina"], socket.assigns.social_stamina)
      )
      |> assign(
        :stamina_regen_rate,
        parse_int(params["stamina_regen_rate"], socket.assigns.stamina_regen_rate)
      )
      ...
      |> assign(
        :illness_severity,
        parse_int(params["illness_severity"], socket.assigns.illness_severity)
      )
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-gray-950 text-gray-100">
      ...
      <nav class="border-b border-gray-800 bg-gray-900 px-6 py-3 flex items-center gap-6">
        ...
        <div class="flex items-center gap-4 ml-4">
          ...
          <.link
            ...
            class="text-sm text-amber-400 font-semibold border-b border-amber-400 pb-0.5"
          >
            New NPC
          </.link>
          <.link
      ...
      <div class="max-w-3xl mx-auto px-6 py-8">
        ...
        <div
          :if={@errors != []}
          class="mb-4 p-3 rounded-lg bg-red-900/30 border border-red-800/50 text-red-400 text-sm"
        >
          <%= for err <- @errors do %>
            <div>{err}</div>
          <% end %>
          ...
          <div class="p-6 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_personality" class="space-y-5">
              <div>
                ...
                <select
                  ...
                  class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-amber-500"
                >
                  <option value="secure" selected={@attachment_style == "secure"}>
                    Secure — trusts bonds, comfortable with closeness
                  </option>

                  <option value="anxious" selected={@attachment_style == "anxious"}>
                    Anxious — fears abandonment, seeks constant reassurance
                  </option>

                  <option value="avoidant" selected={@attachment_style == "avoidant"}>
                    Avoidant — suppresses need for closeness, self-reliant
                  </option>

                  <option value="disorganized" selected={@attachment_style == "disorganized"}>
                    Disorganized — craves and fears intimacy simultaneously
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  <%= for style <- ~w(none dry sarcastic warm dark absurdist) do %>
                    <option value={style} selected={@humor_style == style}>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="2"
                  class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-amber-500"
                  placeholder="How do they talk? Terse? Verbose? Formal? Folksy?"
                >{@speech_style}</textarea>
              </div>
            </form>
            ...
            <div class="mt-5">
              ...
              <p class="text-[10px] text-gray-500 mb-2">
                These shape how the character acts, not a diagnosis of them.
              </p>
              ...
              <div class="grid grid-cols-2 gap-2">
                <%= for %{key: trait, label: label, blurb: blurb} <- TraitCatalog.all() do %>
                  ...
                  >
                    <div class={"w-2 h-2 rounded-full shrink-0 #{if active, do: "bg-purple-400", else: "bg-gray-600"}"}>
                    </div>
                    {label}
                  </button>
                <% end %>
          ...
          <div class="p-6 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <div class="mb-5">
              ...
              <div class="flex flex-wrap gap-2 mb-2">
                <%= for v <- @core_values do %>
                  <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-300 text-xs">
                    {v}
                    <button
                      phx-click="remove_core_value"
                      phx-value-value={v}
                      type="button"
                      class="text-amber-500 hover:text-amber-200 ml-0.5"
                    >
              ...
              <form phx-submit="add_core_value" class="flex gap-2">
                <input
                  type="text"
                  value={@core_value_input}
                  phx-change="update_core_value_input"
                  phx-value-value={@core_value_input}
                  name="value"
                ...
                <button
                  type="submit"
                  class="px-3 py-2 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-400 text-sm hover:bg-amber-500/30 transition-colors"
                >
                  Add
                </button>
              </form>
            ...
            <%!-- Baseline Emotions --%>
            <div class="mb-5">
              <label class="block text-xs text-gray-400 mb-3 font-semibold uppercase tracking-wide">
                Baseline Emotions
              </label>
              <div class="space-y-2">
                <%= for {emotion, color} <- [{"anger", "red"}, {"fear", "purple"}, {"stress", "orange"}, {"gratitude", "green"}, {"confidence", "blue"}, {"sadness", "indigo"}] do %>
                  <div class="flex items-center gap-3">
                    ...
                    />
                    <span class="w-8 text-xs text-gray-400 text-right">
                      {Map.get(@baseline_emotions, emotion, 0)}
                    </span>
                  </div>
                <% end %>
              </div>
            ...
            <div class="mb-5">
              ...
              <div class="space-y-2">
                <%= for emotion <- ~w(anger fear sadness shame stress) do %>
                  <div>
                    <label class="block text-[10px] text-gray-500 mb-1 capitalize">
                      {emotion} tell
                    </label>
                    <input
                      type="text"
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <%= for {b, idx} <- Enum.with_index(@beliefs) do %>
              <div class="flex items-center gap-2 mb-2 p-2 bg-gray-800/60 rounded-lg">
                ...
                <span class="text-[10px] text-gray-500 px-2 py-0.5 rounded bg-gray-700">
                  {b["domain"]}
                </span>
                <span class="text-[10px] text-gray-500">{b["conviction"]}</span>
                <button
                  phx-click="remove_belief"
                  phx-value-index={idx}
            ...
            <form phx-change="update_belief_draft" class="space-y-2">
              ...
              <div class="flex gap-2">
                <select
                  name="domain"
                  class="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-purple-500"
                >
                  <%= for d <- ~w(social self world spiritual) do %>
                    <option value={d} selected={@belief_draft["domain"] == d}>
                      {String.capitalize(d)}
                    </option>
                  <% end %>
                </select>
            ...
            <button
              phx-click="add_belief"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-purple-500/20 border border-purple-500/30 text-purple-400 hover:bg-purple-500/30 transition-colors"
            >
              + Add Belief
            </button>
          </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_trigger_draft" class="space-y-2">
              <input
                type="text"
                name="topic"
                value={@trigger_draft["topic"]}
                placeholder="Topic / keyword that triggers"
              ...
              >
                <%= for r <- ~w(anger_spike fear_spike grief_spike pride_surge shame_trigger) do %>
                  <option value={r} selected={@trigger_draft["reaction_type"] == r}>
                    {String.replace(r, "_", " ") |> String.capitalize()}
                  </option>
                <% end %>
              </select>
              <input
                type="text"
                name="flavor_text"
                value={@trigger_draft["flavor_text"]}
                placeholder="Flavor text (how it manifests)"
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-orange-500"
              />
            </form>
            ...
            <button
              ...
              class="mt-2 text-xs px-3 py-1.5 rounded bg-orange-500/20 border border-orange-500/30 text-orange-400 hover:bg-orange-500/30 transition-colors"
            >
              + Add Trigger
            </button>
          </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_moral_line_draft" class="space-y-2">
              <input
                type="text"
                name="principle"
                value={@moral_line_draft["principle"]}
                placeholder="Moral principle (e.g. Never harm a child)"
            ...
            <button
              phx-click="add_moral_line"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-red-500/20 border border-red-500/30 text-red-400 hover:bg-red-500/30 transition-colors"
            >
              + Add Moral Line
            </button>
          </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <%= for {s, idx} <- Enum.with_index(@secrets) do %>
              <div class="flex items-center gap-2 mb-2 p-2 bg-gray-800/60 rounded-lg">
                ...
                <span class={"text-[10px] px-2 py-0.5 rounded border #{secret_risk_class(s["risk_level"])}"}>
                  {s["risk_level"]}
                </span>
                <button
                  phx-click="remove_secret"
                  phx-value-index={idx}
                  class="text-gray-600 hover:text-red-400"
                >
                  <.icon name="hero-x-mark" class="size-3.5" />
            ...
            <form phx-change="update_secret_draft" class="space-y-2">
              <textarea
                ...
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-rose-500"
              >{@secret_draft["secret_text"]}</textarea>
              <div class="flex gap-2">
                <select
                  name="risk_level"
                ...
                >
                  <%= for r <- ~w(low medium high critical) do %>
                    <option value={r} selected={@secret_draft["risk_level"] == r}>
                      {String.capitalize(r)}
                    </option>
                  <% end %>
                </select>
            ...
            <button
              phx-click="add_secret"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-rose-500/20 border border-rose-500/30 text-rose-400 hover:bg-rose-500/30 transition-colors"
            >
              + Add Secret
            </button>
          </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_desire_draft" class="space-y-2">
              <input
                type="text"
                name="desire"
                value={@desire_draft["desire"]}
                placeholder="Desire statement"
              ...
              <div class="flex gap-2 items-center">
                <select
                  name="domain"
                  class="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-green-500"
                >
                  <%= for d <- ~w(revenge connection safety power redemption knowledge belonging) do %>
                    <option value={d} selected={@desire_draft["domain"] == d}>
                      {String.capitalize(d)}
                    </option>
                  <% end %>
                </select>
            ...
            <button
              phx-click="add_desire"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-green-500/20 border border-green-500/30 text-green-400 hover:bg-green-500/30 transition-colors"
            >
              + Add Desire
            </button>
          </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_goal_draft" class="space-y-2">
              ...
              <input
                type="text"
                name="current_step"
                value={@goal_draft["current_step"]}
                placeholder="Current step / next action"
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-blue-500"
              />
            ...
            <button
              phx-click="add_goal"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-blue-500/20 border border-blue-500/30 text-blue-400 hover:bg-blue-500/30 transition-colors"
            >
              + Add Goal
            </button>
          </div>
        ...
        <div :if={@step == 6} class="space-y-5">
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_grief_draft" class="space-y-2">
              <input
                type="text"
                name="subject"
                value={@grief_draft["subject"]}
                placeholder="Subject of grief (e.g. their sister)"
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                >
                  <%= for lt <- ~w(person role belief home ability) do %>
                    <option value={lt} selected={@grief_draft["loss_type"] == lt}>
                      {String.capitalize(lt)}
                    </option>
                  <% end %>
                </select>
                <select
                ...
                >
                  <%= for s <- ~w(denial anger bargaining depression integration) do %>
                    <option value={s} selected={@grief_draft["stage"] == s}>
                      {String.capitalize(s)}
                    </option>
                  <% end %>
                </select>
            ...
            <button
              phx-click="add_grief_arc"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/30 transition-colors"
            >
              + Add Grief Arc
            </button>
          </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <%= for {fa, idx} <- Enum.with_index(@forgiveness_arcs) do %>
              <div class="flex items-center gap-2 mb-2 p-2 bg-gray-800/60 rounded-lg">
                ...
                <button
                  phx-click="remove_forgiveness_arc"
                  phx-value-index={idx}
                  class="text-gray-600 hover:text-red-400"
                >
                  <.icon name="hero-x-mark" class="size-3.5" />
            ...
            <form phx-change="update_forgiveness_draft" class="space-y-2">
              <textarea
                ...
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-rose-500"
              >{@forgiveness_draft["wound_description"]}</textarea>
              <div class="grid grid-cols-2 gap-2">
                <select
                  name="stage"
                  class="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-rose-500"
                >
                  <%= for s <- ~w(fresh festering processing forgiven hardened) do %>
                    <option value={s} selected={@forgiveness_draft["stage"] == s}>
                      {String.capitalize(s)}
                    </option>
                  <% end %>
                </select>
                <select
                  ...
                  class="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-rose-500"
                >
                  <option value="healing" selected={@forgiveness_draft["direction"] == "healing"}>
                    Healing
                  </option>

                  <option value="hardening" selected={@forgiveness_draft["direction"] == "hardening"}>
                    Hardening
                  </option>

                  <option value="neutral" selected={@forgiveness_draft["direction"] == "neutral"}>
                    Neutral
                  </option>
                </select>
              </div>
          ...
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <p class="text-[10px] text-gray-500 mb-3">
              What does this NPC already believe about someone else — another character, or a player once they show up in play?
            </p>
            ...
            <form phx-change="update_tom_draft" class="space-y-2">
              ...
              >
                <option value="" selected={@tom_draft["subject_id"] in [nil, ""]}>
                  Who is this about?
                </option>
              ...
              <input
                type="text"
                name="known_fact"
                value={@tom_draft["known_fact"]}
                placeholder="Known fact about them"
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-cyan-500"
              />
              <div class="flex items-center gap-3">
                <input
                  ...
                  max="100"
                  name="certainty"
                  value={@tom_draft["certainty"]}
                  class="flex-1 accent-cyan-500"
                />
                <span class="text-xs text-gray-400 w-20">
                  certainty {Map.get(@tom_draft, "certainty", 70)}%
                </span>
            ...
            <button
              phx-click="add_tom_entry"
              class="mt-2 text-xs px-3 py-1.5 rounded bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/30 transition-colors"
            >
              + Add Entry
            </button>
          </div>
        ...
        <div :if={@step == 7} class="space-y-5">
          <div class="p-5 rounded-xl border border-gray-800 bg-gray-900">
            ...
            <form phx-change="update_stamina" class="space-y-4">
              ...
              <div>
                <label class="block text-xs text-gray-400 mb-1 font-semibold uppercase tracking-wide">
                  Stamina Regen Rate — {@stamina_regen_rate}/hr
                </label>
                <input
                  type="range"
                  ...
                  min="0"
                  max="50"
                  value={@stamina_regen_rate}
                  class="w-full accent-green-500"
                />
              </div>

              <div class="pt-4 border-t border-gray-800">
                <h3 class="text-xs font-bold text-gray-400 mb-3 uppercase tracking-wide">
                  Somatic State
                </h3>
            ...
            <div class="mt-5 p-4 rounded-lg bg-gray-800/60 border border-gray-700">
              <div class="text-xs text-gray-400 mb-1 font-semibold uppercase tracking-wide">
                Summary
              </div>
              ...
              <div class="flex gap-2 mt-2 flex-wrap">
                ...
                <span class="text-[10px] px-2 py-0.5 rounded bg-gray-700 text-gray-400">
                  {@attachment_style}
                </span>
                <span class="text-[10px] px-2 py-0.5 rounded bg-gray-700 text-gray-400">
                  humor: {@humor_style}
