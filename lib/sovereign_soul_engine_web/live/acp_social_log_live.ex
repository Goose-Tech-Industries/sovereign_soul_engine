          ...
          <div class="flex-1 divide-y divide-gray-800/40">
            <%= if @scenes == [] do %>
              <div class="p-6 text-center text-xs text-gray-600 italic">
                No autonomous scenes found yet. Trigger a tick to start.
              </div>
            <% else %>
              <%= for scene <- @scenes do %>
                <button
                  ...
                  class={[
                    "w-full p-4 text-left transition-colors hover:bg-gray-800/20 flex flex-col gap-1.5",
                    @selected_scene && @selected_scene.id == scene.id &&
                      "bg-gray-800/40 border-l-2 border-amber-400"
                  ]}
                >
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-xs font-bold text-gray-200 truncate">{scene.title}</span>
                    <span class="text-[9px] text-gray-600 shrink-0">
                      {format_time(scene.inserted_at)}
                    </span>
                  </div>
          ...
          <%= if is_nil(@selected_scene) do %>
            <div class="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <p class="text-xs text-gray-600 mt-1">
                Select an autonomous scene from the sidebar to inspect its timeline.
              </p>
            </div>
          <% else %>
            <%!-- Header --%>
              ...
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-[10px] text-gray-500 mt-0.5">
                    Location: {@selected_scene.location} | Status: {@selected_scene.status}
                  </p>
                </div>
            ...
            <div class="flex-1 overflow-y-auto px-6 py-6 space-y-4">
              <%= if @messages == [] do %>
                <div class="text-center text-xs text-gray-600 italic py-12">
                  No messages recorded in this scene.
                </div>
              <% else %>
                <%= for msg <- @messages do %>
