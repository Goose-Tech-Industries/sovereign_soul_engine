defmodule SovereignSoulEngineWeb.ChatSauceLive do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      ...
      |> assign(:w_trigger_draft, %{
        ...
        "intensity_modifier" => 50,
        "flavor_text" => ""
      })
      |> assign(:w_moral_lines, [])
      |> assign(:w_moral_line_draft, %{
        "principle" => "",
        "will_refuse" => true,
    ...
    <div class="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans antialiased">
      ...
      <main class="flex-1 max-w-7xl w-full mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div :if={@success_message || @error_message} class="col-span-1 lg:col-span-3">
          ...
          >
            <.icon name="hero-check-circle" class="size-5 text-emerald-400" /> {@success_message}
          </div>
          ...
          >
            <.icon name="hero-exclamation-triangle" class="size-5 text-red-400" /> {@error_message}
          </div>
        </div>
        ...
        <div :if={@sauce_tab == "characters"} class="col-span-1 lg:col-span-3 flex gap-4 min-h-[70vh]">
          ...
          <aside class="w-56 shrink-0 space-y-1.5 overflow-y-auto">
            <div class="text-[10px] uppercase font-mono text-slate-500 px-2 pb-1 tracking-wider">
              Characters
            </div>
            ...
            <%= if @inspector_char do %>
                    ...
                    <div>
                      <div class="flex justify-between text-[11px] mb-0.5">
                        ...
                        <span class="text-slate-300 font-mono">
                          {if @inspector_emotional,
                            do: Map.get(@inspector_emotional, key) || 0,
                            else: 0}
                        </span>
                      </div>
                      ...
                      <div class="h-1.5 w-full rounded-full bg-slate-800 overflow-hidden">
                        <div
                          ...
                          style={"width: #{if @inspector_emotional, do: Map.get(@inspector_emotional, key) || 0, else: 0}%; background-color: #{color}"}
                        >
                        </div>
                      </div>
                    </div>
                      ...
                      <%= for {label, key, color} <- [{"Hunger", :hunger, "#f59e0b"}, {"Pain", :pain, "#ef4444"}, {"Fatigue", :fatigue, "#a855f7"}, {"Illness", :illness_severity, "#22c55e"}] do %>
                        <div>
                          <div class="flex justify-between text-[11px] mb-0.5">
                            <span class="text-slate-400">{label}</span>
                            <span class="text-slate-300 font-mono">
                              {Map.get(@inspector_somatic, key) || 0}
                            </span>
                          </div>
                          ...
                          <div class="h-1.5 w-full rounded-full bg-slate-800 overflow-hidden">
                            <div
                              ...
                              style={"width: #{Map.get(@inspector_somatic, key) || 0}%; background-color: #{color}"}
                            >
                            </div>
                          </div>
                        </div>
                  ...
                  <div class="p-4 rounded-xl border border-slate-800 bg-slate-900/40 space-y-2">
                    ...
                    <%= if @inspector_soul do %>
                      <div class="text-[11px] text-slate-400 space-y-1 pt-1">
                        <div>
                          Attachment:
                          <span class="text-slate-200">{@inspector_soul.attachment_style}</span>
                        </div>

                        <div>
                          Humor: <span class="text-slate-200">{@inspector_soul.humor_style}</span>
                        </div>

                        <div>
                          Porousness:
                          <span class="text-slate-200">
                            {@inspector_soul.emotional_susceptibility}/100
                          </span>
                        </div>
                      </div>
                    <% end %>
              ...
              <div :if={@inspector_tab == "beliefs"} class="space-y-4">
                      ...
                      <div class="flex items-start justify-between gap-3 p-2 rounded-lg bg-slate-950/60 text-xs">
                        <div class="flex-1">
                          ...
                          <div class="text-slate-500 font-mono">
                            {b.domain} · conviction {b.conviction}/100
                          </div>
                        </div>
                      </div>
                    <% end %>

                    <p :if={@inspector_beliefs == []} class="text-xs text-slate-600 italic">
                      No beliefs loaded.
                    </p>
                  </div>
                </div>
                ...
                <div class="p-4 rounded-xl border border-slate-800 bg-slate-900/40">
                  ...
                  <div class="space-y-2">
                    ...
                    <p :if={@inspector_triggers == []} class="text-xs text-slate-600 italic">
                      No triggers.
                    </p>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div class="p-4 rounded-xl border border-slate-800 bg-slate-900/40">
                    ...
                    <%= for ml <- @inspector_moral_lines do %>
                      <div class="p-2 rounded-lg bg-slate-950/60 text-xs text-slate-200 mb-1">
                        {ml.principle}
                      </div>
                    <% end %>

                    <p :if={@inspector_moral_lines == []} class="text-xs text-slate-600 italic">
                      None.
                    </p>
                  </div>
                    ...
                    <div class="p-3 rounded-lg bg-slate-950/60 text-xs mb-2">
                      ...
                      <div class="h-1.5 w-full rounded-full bg-slate-800 overflow-hidden">
                        <div
                          ...
                          style={"width: #{g.intensity}%"}
                        >
                        </div>
                      </div>

                      <div class="text-slate-500 mt-1">
                        intensity {g.intensity} · loss type {g.loss_type}
                      </div>
                    </div>
                  <% end %>

                  <p :if={@inspector_grief_arcs == []} class="text-xs text-slate-600 italic">
                    No active grief arcs.
                  </p>
                </div>
                  ...
                  <%= for fa <- @inspector_forgiveness_arcs do %>
                    <div class="p-3 rounded-lg bg-slate-950/60 text-xs mb-2">
                      ...
                      <div class="text-slate-500">
                        direction: {fa.direction} · intensity {fa.intensity}
                      </div>
                    </div>
                  <% end %>

                  <p :if={@inspector_forgiveness_arcs == []} class="text-xs text-slate-600 italic">
                    No active forgiveness arcs.
                  </p>
                </div>
              </div>
              <%!-- Goals sub-tab --%>
              <div :if={@inspector_tab == "goals"} class="space-y-4">
                <div class="p-4 rounded-xl border border-slate-800 bg-slate-900/40">
                  ...
                  <p :if={@inspector_active_goals == []} class="text-xs text-slate-600 italic">
                    No active goals.
                  </p>
                </div>
                ...
                <div class="p-4 rounded-xl border border-slate-800 bg-slate-900/40">
                  ...
                  <p :if={@inspector_desires == []} class="text-xs text-slate-600 italic">
                    No desires.
                  </p>
                </div>
              </div>
              ...
              <div :if={@inspector_tab == "memory"} class="space-y-3">
                ...
                <div class="space-y-2 max-h-[55vh] overflow-y-auto">
                  ...
                  <p
                    :if={@inspector_memories == []}
                    class="text-xs text-slate-600 italic py-4 text-center"
                  >
              ...
              <div :if={@inspector_tab == "theory_of_mind"} class="space-y-4">
                <div class="space-y-3">
                      ...
                      <div class="space-y-1.5">
                        <%= for e <- entries do %>
                          <div class="flex items-start gap-2 text-xs">
                            <span class={[
                              "text-[10px] px-1.5 rounded border mt-0.5",
                              e.is_assumption &&
                                "bg-yellow-500/10 text-yellow-400 border-yellow-500/20",
                              !e.is_assumption && "bg-blue-500/10 text-blue-400 border-blue-500/20"
                            ]}>
                  ...
                  <p
                    :if={@inspector_tom_grouped == []}
                    class="text-xs text-slate-600 italic py-4 text-center"
                  >
                    No theory-of-mind entries.
                  </p>
                </div>
                <%!-- Add ToM entry --%>
                <div class="p-4 rounded-xl border border-slate-800 bg-slate-900/40 space-y-3">
                  ...
                  <form
                    phx-change="inspector_update_tom_draft"
                    phx-submit="inspector_save_tom_entry"
                    class="space-y-3"
                  >
                    <div>
                      <label class="text-[10px] text-slate-500 uppercase font-mono">
                        About Character
                      </label>
                      <select
                        name="subject_id"
                        class="w-full bg-slate-900 border border-slate-800 text-xs text-white rounded-lg px-2 py-1.5 mt-1"
                      >
                        <option value="">Select character...</option>

                        <%= for c <- @inspector_tom_all_chars do %>
                          <option value={c.id} selected={@inspector_tom_draft_subject_id == c.id}>
                            {c.name}
                          </option>
                        <% end %>
                      </select>
                    ...
                    <div>
                      <label class="text-[10px] text-slate-500 uppercase font-mono">
                        Known Fact / Assumption
                      </label>
                      <input
                        type="text"
                        name="known_fact"
                        value={@inspector_tom_draft_fact}
                    ...
                    <div class="flex items-center gap-4">
                      <div class="flex-1">
                        <label class="text-[10px] text-slate-500 uppercase font-mono">
                          Certainty — {@inspector_tom_draft_certainty}%
                        </label>
                        <input
                          type="range"
                          name="certainty"
                          min="0"
                          max="100"
                          value={@inspector_tom_draft_certainty}
                      ...
                      <label class="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                        <input
                          ...
                          name="is_assumption"
                          value="true"
                          checked={@inspector_tom_draft_assumption}
                    ...
                    <button
                      type="submit"
                      class="w-full py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-xs font-semibold hover:bg-cyan-500/20 transition-colors"
                    >
                      Save Knowledge Entry
                    </button>
                  </form>

                  <div :if={@inspector_tom_save_result == :ok} class="text-xs text-emerald-400">
                    Saved.
                  </div>

                  <div :if={@inspector_tom_save_result == :error} class="text-xs text-red-400">
                    Select a character and enter a fact.
                  </div>
                </div>
              </div>
            <% else %>
              <div class="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p class="text-sm text-slate-500">
                  Select a character from the roster to inspect their soul data.
                </p>
              </div>
            <% end %>
          </div>
          ...
          <div class="max-w-2xl mx-auto">
            <div class="mb-6 flex items-center justify-between">
              <div>
                ...
                <p class="text-xs text-slate-400">
                  Step {@w_step} of 7 — fill out each section, then submit.
                </p>
              </div>
            </div>
            <%!-- Step progress --%>
            <div class="flex gap-1 mb-6">
              <%= for s <- 1..7 do %>
                <button
                  phx-click="w_goto_step"
                  phx-value-step={s}
                  class={[
                    "h-2 flex-1 rounded-full transition-all",
                    s == @w_step && "bg-amber-400",
                    s < @w_step && "bg-amber-700",
                    s > @w_step && "bg-slate-800"
                  ]}
                >
                </button>
              <% end %>
            ...
            <div
              :if={@w_errors != []}
              class="mb-4 p-3 rounded-xl border border-red-800/50 bg-red-900/20 text-red-400 text-xs space-y-1"
            >
              <%= for err <- @w_errors do %>
                <div>{err}</div>
              <% end %>
            ...
            <div
              :if={@w_step == 1}
              class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-4"
            >
              ...
              <form phx-change="w_update_identity" class="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    name="slug"
                    value={@w_slug}
                    class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-amber-500/50"
              ...
              <form phx-change="w_update_personality" class="space-y-4">
                <div>
                  ...
                  >
                    ...
                    <option value="anxious" selected={@w_attachment_style == "anxious"}>
                      Anxious — fears abandonment
                    </option>

                    <option value="avoidant" selected={@w_attachment_style == "avoidant"}>
                      Avoidant — self-reliant
                    </option>

                    <option value="disorganized" selected={@w_attachment_style == "disorganized"}>
                      Disorganized — craves and fears
                    </option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="range"
                    name="emotional_susceptibility"
                    min="0"
                    max="100"
                    value={@w_emotional_susceptibility}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="2"
                    class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500/50"
                    placeholder="Terse? Verbose? Formal? Folksy?"
                  >{@w_speech_style}</textarea>
                </div>
              </form>

              <div>
                <label class="block text-xs text-slate-400 mb-2 font-semibold uppercase">
                  Personality Flags
                </label>
                <div class="grid grid-cols-2 gap-2">
                  <%= for {trait, active} <- @w_personality_traits do %>
                    <button
                      phx-click="w_toggle_trait"
                    ...
                    >
                      <div class={"w-2 h-2 rounded-full #{if active, do: "bg-purple-400", else: "bg-slate-600"}"}>
                      </div>
            ...
            <div
              :if={@w_step == 3}
              class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-5"
            >
              ...
              <div>
                ...
                <form phx-submit="w_add_core_value" class="flex gap-2">
                  <input
                    type="text"
                    name="value"
                    value={@w_core_value_input}
                    phx-change="w_update_core_value_input"
                  ...
                  <button
                    type="submit"
                    class="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs text-white border border-slate-700"
                  >
                    Add
                  </button>
                </form>
                  ...
                  <%= for {label, key} <- [{"Anger", "anger"}, {"Fear", "fear"}, {"Stress", "stress"}, {"Gratitude", "gratitude"}, {"Confidence", "confidence"}, {"Sadness", "sadness"}] do %>
                    <div class="flex items-center gap-3">
                      ...
                      <input
                        ...
                        min="0"
                        max="100"
                        value={Map.get(@w_baseline_emotions, key, 0)}
                        phx-change="w_update_baseline_emotion"
                        phx-value-emotion={key}
                        class="flex-1 accent-amber-500"
                      />
                      <span class="text-xs text-slate-300 w-8 font-mono">
                        {Map.get(@w_baseline_emotions, key, 0)}
                      </span>
                    </div>
                  <% end %>
                </div>
                ...
                <div class="space-y-2">
                  <%= for {label, key} <- [{"Anger", "anger"}, {"Fear", "fear"}, {"Sadness", "sadness"}, {"Shame", "shame"}, {"Stress", "stress"}] do %>
                    <div class="flex items-center gap-3">
                      ...
                      <input
                        type="text"
                        value={Map.get(@w_physical_tells, key, "")}
                        phx-change="w_update_physical_tell"
                        phx-value-emotion={key}
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_belief_draft" class="grid grid-cols-3 gap-2">
                  <input
                    type="text"
                    name="belief"
                    value={@w_belief_draft["belief"]}
                    placeholder="A truth they hold..."
                  ...
                  <select
                    name="domain"
                    class="bg-slate-950 border border-slate-800 rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none"
                  >
                  ...
                  <div class="col-span-3 flex items-center gap-3">
                    ...
                    <input
                      ...
                      min="0"
                      max="100"
                      value={@w_belief_draft["conviction"]}
                      class="flex-1 accent-amber-500"
                    />
                    <span class="text-xs font-mono text-slate-300">
                      {@w_belief_draft["conviction"]}
                    </span>
                    <button
                      type="button"
                      phx-click="w_add_belief"
                      class="px-3 py-1 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs hover:bg-amber-500/20"
                    >
                      Add
                    </button>
                  </div>
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_trigger_draft" class="grid grid-cols-2 gap-2">
                  <input
                    type="text"
                    name="topic"
                    value={@w_trigger_draft["topic"]}
                    placeholder="Topic that triggers them..."
                  ...
                  <input
                    type="text"
                    name="flavor_text"
                    value={@w_trigger_draft["flavor_text"] || ""}
                    placeholder="Flavor text (optional)"
                    class="col-span-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50"
                  />
                  <button
                    type="button"
                    phx-click="w_add_trigger"
                    class="col-span-2 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-white hover:bg-slate-700"
                  >
                    + Add Trigger
                  </button>
                </form>
                ...
                <div class="space-y-1.5">
                  <%= for {t, i} <- Enum.with_index(@w_triggers) do %>
                    <div class="flex items-center gap-2 p-2 rounded-lg bg-slate-950/60 text-xs">
                      ...
                      <button
                        phx-click="w_remove_trigger"
                        phx-value-index={i}
                        class="ml-auto text-slate-600 hover:text-rose-400"
                      >
                        ×
                      </button>
                    </div>
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_moral_line_draft" class="flex gap-2">
                  <input
                    type="text"
                    name="principle"
                    value={@w_moral_line_draft["principle"]}
                    placeholder="A line they will not cross..."
                    class="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50"
                  />
                  <button
                    type="button"
                    phx-click="w_add_moral_line"
                    class="px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-white hover:bg-slate-700"
                  >
                    Add
                  </button>
                </form>
                ...
                <form phx-change="w_update_secret_draft" class="grid grid-cols-3 gap-2">
                  <input
                    type="text"
                    name="secret_text"
                    value={@w_secret_draft["secret_text"]}
                  ...
                  <button
                    type="button"
                    phx-click="w_add_secret"
                    class="col-span-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-white hover:bg-slate-700"
                  >
                    + Add Secret
                  </button>
                </form>
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_desire_draft" class="grid grid-cols-3 gap-2">
                  <input
                    type="text"
                    name="desire"
                    value={@w_desire_draft["desire"]}
                    placeholder="What do they crave?"
                    class="col-span-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50"
                  />
                  <select
                    name="domain"
                  ...
                  <div class="col-span-3 flex items-center gap-3">
                    ...
                    <input
                      ...
                      min="0"
                      max="100"
                      value={@w_desire_draft["urgency"]}
                      class="flex-1 accent-amber-500"
                    />
                    <span class="text-xs font-mono text-slate-300 w-8">
                      {@w_desire_draft["urgency"]}
                    </span>
                    <button
                      type="button"
                      phx-click="w_add_desire"
                      class="px-3 py-1 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs hover:bg-amber-500/20"
                    >
                      Add
                    </button>
                  ...
                  <%= for {d, i} <- Enum.with_index(@w_desires) do %>
                    <div class="flex items-center gap-2 p-2 rounded-lg bg-slate-950/60 text-xs">
                      ...
                      <span class="text-slate-400 font-mono">
                        {d["domain"]} · urgency {d["urgency"]}
                      </span>
                      <button
                        phx-click="w_remove_desire"
                        phx-value-index={i}
                        class="text-slate-600 hover:text-rose-400"
                      >
                        ×
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_goal_draft" class="space-y-2">
                  ...
                  <input
                    type="text"
                    name="current_step"
                    value={@w_goal_draft["current_step"]}
                    placeholder="Current step toward the goal..."
                    class="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50"
                  />
                  <div class="flex items-center gap-3">
                    ...
                    <input
                      ...
                      min="0"
                      max="100"
                      value={@w_goal_draft["priority"]}
                      class="flex-1 accent-amber-500"
                    />
                    <span class="text-xs font-mono text-slate-300 w-8">
                      {@w_goal_draft["priority"]}
                    </span>
                    <button
                      type="button"
                      phx-click="w_add_goal"
                      class="px-3 py-1 rounded-lg bg-blue-500/10 border border-blue-500/30 text-blue-400 text-xs hover:bg-blue-500/20"
                    >
                      Add
                    </button>
                  </div>
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_grief_draft" class="grid grid-cols-2 gap-2">
                  <input
                    type="text"
                    name="subject"
                    value={@w_grief_draft["subject"]}
                    placeholder="Who / what was lost?"
                    class="bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50"
                  />
                  <select
                    name="loss_type"
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <input
                      ...
                      min="0"
                      max="100"
                      value={@w_grief_draft["intensity"]}
                      class="flex-1 accent-indigo-500"
                    />
                    <span class="text-xs font-mono text-slate-300 w-8">
                      {@w_grief_draft["intensity"]}
                    </span>
                  </div>

                  <button
                    type="button"
                    phx-click="w_add_grief_arc"
                    class="col-span-2 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-white hover:bg-slate-700"
                  >
                    + Add Grief Arc
                  </button>
              ...
              <div class="p-6 rounded-2xl border border-slate-800 bg-slate-900/40 space-y-3">
                ...
                <form phx-change="w_update_forgiveness_draft" class="grid grid-cols-2 gap-2">
                  <input
                    ...
                    value={@w_forgiveness_draft["wound_description"]}
                    placeholder="The wound / betrayal..."
                    class="col-span-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500/50"
                  />
                  <select
                    name="stage"
                  ...
                  <select
                    ...
                    class="bg-slate-950 border border-slate-800 rounded-lg px-2 py-1.5 text-xs text-white"
                  >
                    <option value="neutral" selected={@w_forgiveness_draft["direction"] == "neutral"}>
                      Neutral
                    </option>

                    <option value="healing" selected={@w_forgiveness_draft["direction"] == "healing"}>
                      Healing
                    </option>

                    <option
                      value="hardening"
                      selected={@w_forgiveness_draft["direction"] == "hardening"}
                    >
                      Hardening
                    </option>
                  </select>
                  <button
                    type="button"
                    phx-click="w_add_forgiveness_arc"
                    class="col-span-2 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-white hover:bg-slate-700"
                  >
                    + Add Forgiveness Arc
                  </button>
                ...
                <div class="space-y-1.5">
                  <%= for {fa, i} <- Enum.with_index(@w_forgiveness_arcs) do %>
                    <div class="flex items-center gap-2 p-2 rounded-lg bg-slate-950/60 text-xs">
                      ...
                      <span class="text-rose-400 font-mono">{fa["stage"]}</span>
                      <button
                        phx-click="w_remove_forgiveness_arc"
                        phx-value-index={i}
                        class="text-slate-600 hover:text-rose-400"
                      >
                        ×
            ...
            >
              ...
              <form phx-change="w_update_stamina" class="space-y-3">
                <div class="grid grid-cols-3 gap-3">
                  <%= for {label, name, val} <- [{"Social Stamina", "social_stamina", @w_social_stamina}, {"Stamina Max", "stamina_max", @w_stamina_max}, {"Regen/hr", "stamina_regen_rate", @w_stamina_regen_rate}] do %>
                    <div>
                      <label class="block text-[10px] text-slate-400 mb-1 uppercase font-mono">
                        {label}
                      </label>
                      <input
                        type="number"
              ...
              <div class="pt-2 space-y-2">
                <p class="text-xs text-slate-400">
                  Everything looks good? Click Create to initialize the full soul.
                </p>

                <button
                  phx-click="w_create_npc"
                  class="w-full py-3 rounded-xl font-bold bg-gradient-to-tr from-amber-500 to-orange-400 text-slate-950 hover:shadow-lg hover:shadow-amber-500/20 transition-all"
                >
        ...
        <div :if={@sauce_tab == "social"} class="col-span-1 lg:col-span-3 flex gap-4 min-h-[70vh]">
          <aside class="w-64 shrink-0 overflow-y-auto space-y-0.5">
            <div class="flex items-center justify-between mb-2">
              <span class="text-[10px] uppercase font-mono text-slate-500 px-1">
                Autonomous Scenes
              </span>
              <div class="flex gap-1">
                <button
                  phx-click="social_refresh"
                  class="text-[10px] text-slate-500 hover:text-slate-300 px-2 py-1 rounded border border-slate-800 hover:border-slate-600"
                >
                  Refresh
                </button>
                <button
                  phx-click="social_trigger_tick"
                  class="text-[10px] text-amber-400 hover:text-amber-300 px-2 py-1 rounded border border-amber-500/30 hover:border-amber-500/50"
                >
                  Trigger Tick
                </button>
              </div>
            ...
            <%= if @social_scenes == [] do %>
              <div class="text-center p-6 text-xs text-slate-600 italic">
                No autonomous scenes yet.
              </div>
            <% else %>
              <%= for scene <- @social_scenes do %>
                <button
                  ...
                  class={[
                    ...
                    @social_selected && @social_selected.id == scene.id &&
                ...
                >
                  ...
                  <div class="text-[10px] text-slate-500">
                    {scene.status} · {format_time(scene.inserted_at)}
                  </div>
                </button>
              <% end %>
            <% end %>
            ...
            <%= if @social_selected do %>
              <div class="flex items-center gap-3 p-4 border-b border-slate-800 bg-slate-900/30 rounded-t-xl">
                <div class="flex-1">
                  ...
                  <div class="text-[10px] text-slate-500">
                    {@social_selected.location} · {@social_selected.status}
                  </div>
                </div>
              ...
              <div class="flex-1 overflow-y-auto p-4 space-y-3 max-h-[60vh]">
                <%= if @social_messages == [] do %>
                  <div class="text-center text-xs text-slate-600 italic py-8">
                    No messages in this scene.
                  </div>
                <% else %>
                    ...
                    <div class="flex gap-3 max-w-[85%]">
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="px-3 py-2 rounded-2xl rounded-tl-md bg-slate-900 border border-slate-800 text-xs leading-relaxed text-slate-200">
                          ...
                          <div
                            :if={msg.private_thought && msg.private_thought != ""}
                            class="mt-1.5 pt-1 border-t border-purple-500/20 text-[10px] text-purple-400 font-mono"
                          >
  ...
  def handle_event("switch_sauce_tab", %{"tab" => "social"}, socket) do
    ...
    {socket, messages} =
      case scenes do
        ...
        _ ->
          {assign(socket, :social_selected, nil), []}
      end

    {:noreply,
     socket
     |> assign(:sauce_tab, "social")
     |> assign(:social_scenes, scenes)
     |> assign(:social_messages, messages)}
  end
  ...
  def handle_event("social_select_scene", %{"id" => id}, socket) do
    case Enum.find(socket.assigns.social_scenes, &(&1.id == id)) do
      ...
      scene ->
        ...
        {:noreply,
         socket |> assign(:social_selected, scene) |> assign(:social_messages, messages)}
    end
  end
  ...
  def handle_event("inspector_update_tom_draft", params, socket) do
    {:noreply,
     socket
     |> assign(
       :inspector_tom_draft_fact,
       Map.get(params, "known_fact", socket.assigns.inspector_tom_draft_fact)
     )
     |> assign(
       :inspector_tom_draft_certainty,
       parse_int(params["certainty"], socket.assigns.inspector_tom_draft_certainty)
     )
     |> assign(
       :inspector_tom_draft_assumption,
       Map.get(params, "is_assumption", "true") == "true"
     )
     |> assign(
       :inspector_tom_draft_subject_id,
       Map.get(params, "subject_id", socket.assigns.inspector_tom_draft_subject_id)
     )}
  end
  ...
  @impl true
  def handle_event("w_next_step", _params, %{assigns: %{w_step: s}} = socket)
      when s < @wizard_steps,
  ...
  def handle_event("w_update_personality", params, socket) do
    {:noreply,
     ...
     |> assign(
       :w_attachment_style,
       Map.get(params, "attachment_style", socket.assigns.w_attachment_style)
     )
     |> assign(:w_humor_style, Map.get(params, "humor_style", socket.assigns.w_humor_style))
     |> assign(
       :w_emotional_susceptibility,
       parse_int(params["emotional_susceptibility"], socket.assigns.w_emotional_susceptibility)
     )
  ...
  def handle_event("w_toggle_trait", %{"trait" => trait}, socket) do
    traits =
      Map.put(
        socket.assigns.w_personality_traits,
        trait,
        !Map.get(socket.assigns.w_personality_traits, trait, false)
      )
  ...
  def handle_event("w_add_core_value", _params, socket) do
    ...
    if val != "" and val not in socket.assigns.w_core_values do
      {:noreply,
       socket
       |> assign(:w_core_values, socket.assigns.w_core_values ++ [val])
       |> assign(:w_core_value_input, "")}
    else
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_baseline_emotions,
         Map.put(socket.assigns.w_baseline_emotions, e, parse_int(v, 0))
       )}
  ...
  def handle_event("w_update_physical_tell", %{"emotion" => e, "value" => v}, socket),
    do:
      {:noreply,
       assign(socket, :w_physical_tells, Map.put(socket.assigns.w_physical_tells, e, v))}

  @impl true
  def handle_event(
        "w_update_transference",
        %{"index" => idx, "field" => field, "value" => val},
        socket
      ) do
    entries =
      List.update_at(
        socket.assigns.w_transference_entries,
  ...
  def handle_event("w_update_belief_draft", params, socket) do
    draft =
      socket.assigns.w_belief_draft
      |> Map.merge(Map.take(params, ["belief", "domain"]))
      |> Map.put(
        "conviction",
        parse_int(params["conviction"], socket.assigns.w_belief_draft["conviction"])
      )
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_beliefs,
         List.delete_at(socket.assigns.w_beliefs, String.to_integer(idx))
       )}

  @impl true
  def handle_event("w_update_trigger_draft", params, socket) do
    draft =
      socket.assigns.w_trigger_draft
      |> Map.merge(Map.take(params, ["topic", "reaction_type", "flavor_text"]))
      |> Map.put(
        "intensity_modifier",
        parse_int(
          params["intensity_modifier"],
          socket.assigns.w_trigger_draft["intensity_modifier"]
        )
      )
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_triggers,
         List.delete_at(socket.assigns.w_triggers, String.to_integer(idx))
       )}
  ...
  def handle_event("w_add_moral_line", _params, socket) do
    if String.trim(socket.assigns.w_moral_line_draft["principle"] || "") != "" do
      {:noreply,
       ...
       |> assign(
         :w_moral_lines,
         socket.assigns.w_moral_lines ++ [socket.assigns.w_moral_line_draft]
       )
       |> assign(:w_moral_line_draft, %{
         "principle" => "",
         "will_refuse" => true,
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_moral_lines,
         List.delete_at(socket.assigns.w_moral_lines, String.to_integer(idx))
       )}
  ...
  def handle_event("w_update_secret_draft", params, socket) do
    draft =
      Map.merge(
        socket.assigns.w_secret_draft,
        Map.take(params, ["secret_text", "risk_level", "domain"])
      )
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_secrets,
         List.delete_at(socket.assigns.w_secrets, String.to_integer(idx))
       )}

  @impl true
  def handle_event("w_update_desire_draft", params, socket) do
    draft =
      socket.assigns.w_desire_draft
      |> Map.merge(Map.take(params, ["desire", "domain"]))
      |> Map.put(
        "urgency",
        parse_int(params["urgency"], socket.assigns.w_desire_draft["urgency"])
      )
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_desires,
         List.delete_at(socket.assigns.w_desires, String.to_integer(idx))
       )}

  @impl true
  def handle_event("w_update_goal_draft", params, socket) do
    draft =
      socket.assigns.w_goal_draft
      |> Map.merge(Map.take(params, ["goal", "current_step"]))
      |> Map.put(
        "priority",
        parse_int(params["priority"], socket.assigns.w_goal_draft["priority"])
      )
  ...
  def handle_event("w_remove_goal", %{"index" => idx}, socket),
    do:
      {:noreply,
       assign(socket, :w_goals, List.delete_at(socket.assigns.w_goals, String.to_integer(idx)))}

  @impl true
  def handle_event("w_update_grief_draft", params, socket) do
    draft =
      socket.assigns.w_grief_draft
      |> Map.merge(Map.take(params, ["subject", "loss_type", "stage"]))
      |> Map.put(
        "intensity",
        parse_int(params["intensity"], socket.assigns.w_grief_draft["intensity"])
      )
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_grief_arcs,
         List.delete_at(socket.assigns.w_grief_arcs, String.to_integer(idx))
       )}

  @impl true
  def handle_event("w_update_forgiveness_draft", params, socket) do
    draft =
      socket.assigns.w_forgiveness_draft
      |> Map.merge(Map.take(params, ["wound_description", "stage", "direction"]))
      |> Map.put(
        "intensity",
        parse_int(params["intensity"], socket.assigns.w_forgiveness_draft["intensity"])
      )
  ...
  def handle_event("w_add_forgiveness_arc", _params, socket) do
    if String.trim(socket.assigns.w_forgiveness_draft["wound_description"] || "") != "" do
      {:noreply,
       ...
       |> assign(
         :w_forgiveness_arcs,
         socket.assigns.w_forgiveness_arcs ++ [socket.assigns.w_forgiveness_draft]
       )
       |> assign(:w_forgiveness_draft, %{
         "wound_description" => "",
         "stage" => "fresh",
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_forgiveness_arcs,
         List.delete_at(socket.assigns.w_forgiveness_arcs, String.to_integer(idx))
       )}

  @impl true
  def handle_event("w_update_tom_draft", params, socket) do
    draft =
      socket.assigns.w_tom_draft
      |> Map.merge(Map.take(params, ["known_fact"]))
      |> Map.put(
        "certainty",
        parse_int(params["certainty"], socket.assigns.w_tom_draft["certainty"])
      )
    ...
    do:
      {:noreply,
       assign(
         socket,
         :w_tom_entries,
         List.delete_at(socket.assigns.w_tom_entries, String.to_integer(idx))
       )}
  ...
  def handle_event("w_update_stamina", params, socket) do
    {:noreply,
     ...
     |> assign(
       :w_social_stamina,
       parse_int(params["social_stamina"], socket.assigns.w_social_stamina)
     )
     |> assign(
       :w_stamina_regen_rate,
       parse_int(params["stamina_regen_rate"], socket.assigns.w_stamina_regen_rate)
     )
     ...
     |> assign(
       :w_illness_severity,
       parse_int(params["illness_severity"], socket.assigns.w_illness_severity)
     )}
  end
  ...
  def handle_event("w_create_npc", _params, socket) do
    ...
    with {:ok, character} <-
           Characters.create_character(%{
             ...
             slug: a.w_slug,
             kind: a.w_kind,
             description: a.w_description,
             status: a.w_status
           }),
         active_traits =
           a.w_personality_traits
           |> Enum.filter(fn {_, v} -> v end)
           |> Enum.map(fn {k, _} -> k end),
         transference =
           a.w_transference_entries
         ...
         {:ok, _soul} <-
           Souls.create_soul_profile(%{
             ...
             attachment_style: a.w_attachment_style,
             humor_style: a.w_humor_style,
             emotional_susceptibility: a.w_emotional_susceptibility,
             speech_style: a.w_speech_style,
             personality_traits: Map.new(active_traits, &{&1, true}),
             core_values: a.w_core_values,
             baseline_emotions: a.w_baseline_emotions,
             physical_tells: a.w_physical_tells,
             transference_profile: transference,
             social_stamina: a.w_social_stamina,
             stamina_regen_rate: a.w_stamina_regen_rate,
             stamina_max: a.w_stamina_max
           }),
         {:ok, _emotion} <-
         ...
         {:ok, _somatic} <-
           Souls.create_somatic_state(%{
             ...
             pain: a.w_pain,
             fatigue: a.w_fatigue,
             illness_severity: a.w_illness_severity
           }) do
      for b <- a.w_beliefs,
          do:
            Souls.create_belief(%{
  ...
  defp cog_badge(score) when score >= 70,
    do: {"bg-red-500/20 text-red-400 border-red-500/30", "HIGH"}

  defp cog_badge(score) when score >= 40,
    do: {"bg-yellow-500/20 text-yellow-400 border-yellow-500/30", "MED"}
  ...
  defp load_inspector_char(socket, id) do
    ...
    {cog_score, _stressors} =
      SovereignSoulEngine.Souls.CognitiveLoad.compute(
        emotional,
        somatic,
        grief_arcs,
        active_goals
  ...
  defp load_inspector_tab(socket, "arcs", _id) do
    ...
    assign(
      socket,
      :inspector_forgiveness_arcs,
      Souls.list_active_forgiveness_arcs_for_character(id)
    )
  end
