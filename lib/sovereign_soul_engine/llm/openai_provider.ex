defmodule SovereignSoulEngine.LLM.OpenAIProvider do
  @moduledoc """
  ...
  defp send_request(api_key, body) do
    ...
    case Req.post(@base_url,
      ...
      {:ok, %{status: status, body: response_body}} ->
        Logger.warning("OpenAI API returned status #{status}: #{inspect(response_body)}")
        {:error, "openai_http_#{status}"}

      {:error, %{reason: reason}} ->
        Logger.warning("OpenAI API request failed: #{inspect(reason)}")
        {:error, "openai_request_failed: #{inspect(reason)}"}
    end
  end
  ...
  defp parse_response(response) do
    Logger.warning("Unexpected OpenAI response shape: #{inspect(response)}")
